...
class DenseNet161(GeneralizedTorchvisionClassifier):

    def __init__(self, project, config, dbConnector, fileServer, options):
        super().__init__(project, config, dbConnector, fileServer, options)
        assert self.detectron2cfg.MODEL.TVCLASSIFIER.FLAVOR == 'densenet161', \
            f'{self.detectron2cfg.MODEL.TVCLASSIFIER.FLAVOR} != "densenet161"'

    @classmethod
    def getDefaultOptions(cls):
        opts = GenericDetectron2Model._load_default_options(
            'config/ai/model/detectron2/labels/densenet161.json',
            DEFAULT_OPTIONS
        )
        opts['defs']['model'] = 'labels/densenet161_ImageNet.yaml'
        return opts