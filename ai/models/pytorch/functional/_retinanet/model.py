...
class RetinaNet(nn.Module):
    ...
    def updateModel(self, labelClasses, addMissing=False, removeObsolete=False):
        ...
        if addMissing and len(classes_missing):
            weights = self.cls_head[-1].weight
            biases = self.cls_head[-1].bias
            ...
            self.cls_head[-1].weight = nn.Parameter(weights)
            self.cls_head[-1].bias = nn.Parameter(biases)

            print(f'Neurons for {len(classes_missing)} new label classes added to RetinaNet model.')
        ...
        if removeObsolete and len(classes_obsolete):
            ...
            for classname in classes_obsolete:
                ...
                startIdx = classIdx * self.numAnchors
                endIdx = startIdx + self.numAnchors
                weights = torch.cat((weights[0:startIdx,...], weights[endIdx:,...]), 0)
                biases = torch.cat((biases[0:startIdx], biases[endIdx:]), 0)
        ...
        self.numClasses = len(self.labelclassMap)
