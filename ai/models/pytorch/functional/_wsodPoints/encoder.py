...
class DataEncoder:
    ...
    def encode(self, points, labels, target_size):
        '''
            ...
            Args:
                ...
                target_size (int/tuple): model prediction grid size (width, height)
        ...
        # prepare target grid
        loc_targets = torch.LongTensor(self.num_classes + 1, target_size[0], target_size[1]).zero_()

        if not len(labels):
            return loc_targets

        # distribute labels in grid
        positions = (points * torch.tensor(target_size).float()).long()
    ...
    def decode(self, loc_preds, min_conf=0.5, nms_dist=2):
        ...
        points = torch.nonzero(loc_preds >= min_conf)[:,1:]
        labels = torch.argmax(confidences, 1).squeeze()
