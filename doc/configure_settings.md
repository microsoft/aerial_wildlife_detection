...
| demoMode | 'yes', 'no' | 'no' |  | Whether to run AIde in demo mode or not. In demo mode, the platform behaves as follows: - User accounts are disabled; visitors automatically redirected to the interface page - No annotations are being stored (predictions can be added off-line, though) - Upon clicking "Next", images are shown in random order - The "Previous" and "Log out" buttons are disabled and hidden - The _AIController_ module is disabled |
